...
public abstract class CameraBridgeViewBase extends SurfaceView implements SurfaceHolder.Callback {
    ...
    private void updateMatrix() {
        float mWidth = this.getWidth();
        float mHeight = this.getHeight();
        float hWidth = this.getWidth() / 2.0f;
        float hHeight = this.getHeight() / 2.0f;
        float cWidth = (float) Resources.getSystem().getDisplayMetrics().widthPixels;
        float cHeight = (float) Resources.getSystem().getDisplayMetrics().heightPixels;

        float scale = cWidth / (float) mHeight;
        float scale2 = cHeight / (float) mWidth;
        if (scale2 > scale) {
            scale = scale2;
        }
        ...
        mMatrix.reset();
        if (isFrontCamera) {
            mMatrix.preScale(-1, 1, hWidth, hHeight);
        }
        mMatrix.preTranslate(hWidth, hHeight);
        if (isFrontCamera) {
        ...
        mMatrix.preTranslate(-hWidth, -hHeight);
        mMatrix.preScale(scale, scale, hWidth, hHeight);
    }
